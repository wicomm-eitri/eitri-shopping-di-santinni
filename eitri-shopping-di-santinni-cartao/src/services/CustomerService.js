...
// TODO: no emulador o cartao ainda não compartilha o sharedStorage com os outros apps (só existe local), então o token do login não é
// encontrado e o favoritar abre o login. Validar de novo quando o app estiver registrado no mesmo grupo dos demais no Console da Eitri
...
export const productOnWishlist = async identifier => {
	try {
		...
		const itemExists = products.find(
			item => String(item.productId) === String(identifier) || String(item.id) === String(identifier)
		)
...
export const removeItemFromWishlist = async identifier => {
	try {
		...
		const updatedProducts = (targetList.products || []).filter(
			item => String(item.ID) !== String(identifier) && String(item.notes) !== String(identifier)
		)
